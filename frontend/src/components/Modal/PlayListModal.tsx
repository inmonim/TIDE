...
interface PlayListCreateAPIInterFace {
...
const DiaryListModal: FC<PlayListModalProps> = props => {
  ...
  useEffect(() => {
    switch (create_status.status) {
      case 'completed':
        toast.success('플레이리스트 생성 성공');
        dispatch(playListMineAsync());
        dispatch(playListCreateinitStatus());
        getModalType(0);
        break;
      case 'failed':
        toast.error('플레이리스트 생성 실패');
        dispatch(playListCreateinitStatus());
  ...
  const onChangeInputPublic = (event: React.ChangeEvent<HTMLInputElement>) => {
    // 0이 공개 1이 비공개
    if (event.target.checked === false) setIsPublic('0');
    else setIsPublic('1');
    // console.log(playlistTitle, isPublic)
  };

  const onListCreate = (playlistInfo: PlayListCreateAPIInterFace) => {
  ...
  const onListEdit = (playlistInfo: PlayListAPIInterFace) => {
    ...
    if (
      !isNaN(Number(playlistInfo.playlistId)) &&
            ...
            {type === 1 ? (
              <div className={`justify-center items-center flex flex-col h-28`}>
                <form className={`w-full `}>
                  <label
                    ...
                    <input
                      ...
                      type="text"
                      id="title"
                      className={`ml-3 w-[80%] rounded-md text-black p-1`}></input>
                  </label>
                  <label
                    className={` p-3 select-none w-full flex justify-start whitespace-nowrap items-center`}>
                    <p> 공개 여부 </p>
                    <div className={`flex  ml-16 gap-x-5 text-blue-300`}>
                      <input
                        onChange={onChangeInputPublic}
                      ...
                      />
                      <p className={`${isPublic === '0' ? `` : `text-black`}`}>
                        {' '}
                        {isPublic === '0' ? '공개' : '비공개'}{' '}
                      </p>
                    </div>
                ...
                <button
                  ...
                  className={`border rounded-xl bg-slate-700 hover:bg-blue-900 duration-200 h-[40px] w-[100px] md:mt-2`}>
                  {' '}
                  생성{' '}
                </button>
              </div>

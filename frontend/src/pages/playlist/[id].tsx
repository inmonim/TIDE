...
const PlayListDetail: FC<PlayListProps> = props => { 
  ...
  return (
    <>
      ...
      <PlayListModal type={playlistType} getModalType={getModalType} playlistId={Number(playListId)}/>
      ...
      p-[4rem] pt-[2rem] lg12:pr-[calc(200px)] lg12:pl-[calc(15%+100px)] pb-[240px] text-[#eeeeee] flex flex-col min-h-[100vh] pt-[calc(2rem+40px)] bg-gradient-to-t from-blue-900 to-slate-900 `}>
        <div className={`text-[0.85rem] w-[100%] z-[2] select-none h-[100%]`}>
          ...
          <h1 className="text-lg md:text-5xl font-bold md:ml-[2rem] md86:ml-0"> {router.query.playlistTitle} </h1>
          <div className={`grid gap-2 grid-cols-2`}>
            <button 
            onClick={()=>setPlaylistType(2)}
            className={`text-md border pl-2 pr-2 rounded-lg h-8 bg-gray-800 hover:bg-gray-600 duration-200`}> 수정 </button>
            <button 
            onClick={playListDel}
